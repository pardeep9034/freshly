...
import { Box, Typography, TableContainer, Table, TableHead, TableRow, TableCell, TableBody, Modal } from "@mui/material";
import { formatDistanceToNow, parseISO } from "date-fns";

const ShowOrders = () => {
  const [selectedOrder, setSelectedOrder] = useState(null); // Track selected order for modal
  const { orders } = useCart(); // Store orders
      ...
      <table className="table-auto w-full text-left border-collapse">
        ...
        <tbody>
          {orders.slice().reverse().map((order,index) => (
            ...
            >
              <td className="p-2">ORD00{orders.length-index}</td>
